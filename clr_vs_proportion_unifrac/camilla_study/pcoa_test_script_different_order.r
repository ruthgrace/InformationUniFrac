...
breastmilk.otu.tab <- read.table("./td_OTU_tag_mapped_lineage.txt", header=T, sep="\t", row.names=1, comment.char="", check.names=FALSE)
...
breastmilk.tree <- read.tree("./fasttree_all_seed_OTUs.tre")
breastmilk.tree <- midpoint(breastmilk.tree)

# read metadata
MyMeta<- read.table("./metadata.txt", header=T, sep="\t", row.names=1, comment.char="", check.names=FALSE)
...
#save plots as PDF
pdf("camilla_breastmilk_pcoa_plots_different_order.pdf")
