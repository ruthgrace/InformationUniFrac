...
writeFile <- function(saliva,stool,ear,tree,filename,condition1,condition2,condition3,otuIDs) {
	#concatenate	
	data <- data.frame(saliva,stool,ear)
	colnames(data) <- sub("^X", "", colnames(data))
	rownames(data) <- otuIDs

	#make condition vector
	groups <- as.factor(c(rep(condition1,ncol(saliva)),rep(condition2,ncol(stool)),rep(condition3,ncol(ear))))
...
ear.otu <- apply(ear.otu, 1, function(x){as.numeric(x)})

# remove all samples with read count lower than 10,000
saliva.sum <- apply(saliva.otu,2,sum)
stool.sum <- apply(stool.otu,2,sum)
ear.sum <- apply(ear.otu,2,sum)

saliva.low <- saliva.otu[,which(saliva.sum<3000)]
stool.low <- stool.otu[,which(stool.sum<3000)]
ear.low <- ear.otu[,which(ear.sum<3000)]

saliva.med <- saliva.otu[,which((saliva.sum>3000) & (saliva.sum < 6000))]
stool.med <- stool.otu[,which((stool>3000) & (stool.sum < 6000))]
ear.med <- ear.otu[,which((ear>3000) & (ear.sum < 6000))]

saliva.high <- saliva.otu[,which(saliva.sum>6000)]
stool.high <- stool.otu[,which(stool.sum>6000)]
ear.high <- ear.otu[,which(ear.sum>6000)]

writeFile(saliva.low,stool.low,ear.low,tree,"low_sequencing_depth",saliva,stool,ear,otuIDs)
writeFile(saliva.med,stool.med,ear.med,tree,"med_sequencing_depth",saliva,stool,ear,otuIDs)
writeFile(saliva.high,stool.high,ear.high,tree,"high_sequencing_depth",saliva,stool,ear,otuIDs)
